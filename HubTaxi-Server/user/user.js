...
console.log("asd42")
        ...
        console.log("getUserDevice1")
        User.dbProvider[table].findOne(findConditions, function (err, user) {
                cb(err, user);
            });
            console.log(User.dbProvider[table].findOne(findConditions))
            console.log("getUserDevice2")
        };
console.log("as6d2")
        ...
        console.log("authenticateDriver")
            var user;
            Utils.logInfo('Authenticating starting...');
            Utils.logInfo('siteId: ', siteId);
            Utils.logInfo('username: ', userName);
            Utils.logInfo('password: ', passWord);
            console.log("authenticateDriver2")
            if (userName === 'anon') {
                user = {};
                user.site = siteId;
                user.id = '0';
                ...
                user.role = 'anon';
                callback(null, user);
                console.log("authenticateDriver3")
            } else {
            console.log("authenticateDriver4")
                async.waterfall([
                    function (cb) {
                    console.log("authenticateDriver5")
                        User.getDriver(siteId, userName.toLowerCase(), function (error, user) {
                            cb(error, user);
                            console.log("authenticateDriver7")
                            console.log(user)
                        });

                        console.log("authenticateDriver6")
                    }, function (user, cb) {
                    console.log("authenticateDriver22")
                        var errMsg;
                        if (user) {
console.log("authenticateDriver8")
                            hash(passWord, user.salt, function (err, hash) {
                                cb(err, user, hash);
                            });
                        } else {
                        console.log("authenticateDriver9")
                            errMsg = 'LOGIN.ERROR.USERNAME';
                            cb(errMsg, null);
                            Utils.logInfo('Authenticating', 'error', errMsg);
                        }
                    }, function (user, hash, cb) {
                    console.log("authenticateDriver10")
                        var errMsg;
                        if (hash === user.hash) {
                            user.password = passWord;
                            cb(null, user);
                ...
                console.log("authenticateDriver11")
                    callback(error, result);
                });
            }
        };
        ...
        console.log("driverAuthenticateSuccess")
            var client_ip, expired_date, token, user;
            expired_date = new Date();
            expired_date.setMinutes(expired_date.getMinutes() + 6 * 60);
            client_ip = Utils.getClientIp(req);
